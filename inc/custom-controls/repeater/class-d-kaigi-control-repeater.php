...
if ( ! class_exists( 'd_kaigi_Control_Repeater' ) ) {
	/**
	 * Repeater control
	 */
	class D_Kaigi_Control_Repeater extends WP_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array() ) {
			...
			if ( empty( $this->button_label ) ) {
				$this->button_label =
				/* translators: %s: '' */
				sprintf( esc_attr__( 'Add new %s', 'd-kaigi' ), $this->row_label['value'] );
			}
		...
		public function enqueue() {
			...
			wp_enqueue_script(
				'd-kaigi-repeater',
				get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.js',
			...
			wp_enqueue_style(
				'd-kaigi-repeater',
				get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.css',
				array(),
				null,
			);
		}
		...
		protected function render_content() {
			...
			<?php if ( isset( $this->choices['limit'] ) ) : ?>
				<p class="limit">
					<?php
					/* translators: %s: '' */
					printf( esc_attr__( 'Limit: %s rows', 'd-kaigi' ), esc_html( $this->choices['limit'] ) );
					?>
				</p>
			<?php endif; ?>
			...
			<script type="text/html" class="customize-control-repeater-content">
				...
				<li class="repeater-row minimized" data-row="{{{ index }}}">
					...
					<div class="repeater-row-content">
						<# _.each( data, function( field, i ) { #>
							...
							<div class="repeater-field repeater-field-{{{ field.type }}}">
								...
								<# } else if ( 'color' === field.type ) { #>
									...
									<label>
										...
										<# if ( field.description ) { #>
											<span class="description customize-control-description">{{{ field.description }}}</span>
										<# } #>
										<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'd-kaigi' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
								...
								<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
									...
									<figure class="d-kaigi-image-attachment" data-placeholder="<?php echo esc_attr__( 'No Image Selected', 'theme-slug' ); ?>" >
										<# if ( field.default ) { #>
											<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
											<img src="{{{ defaultImageURL }}}">
										<# } else { #>
											<?php echo esc_attr__( 'No Image Selected', 'theme-slug' ); ?>
										<# } #>
									</figure>

									<div class="actions">
										<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php echo esc_attr__( 'Remove', 'theme-slug' ); ?></button>
										<button type="button" class="button upload-button" data-label=" <?php echo esc_attr__( 'Add Image', 'theme-slug' ); ?>" data-alt-label="<?php echo esc_attr__( 'Change Image', 'theme-slug' ); ?>" >
											<# if ( field.default ) { #>
												<?php echo esc_attr__( 'Change Image', 'theme-slug' ); ?>
											<# } else { #>
												<?php echo esc_attr__( 'Add Image', 'theme-slug' ); ?>
											<# } #>
										</button>
										<# if ( field.default.id ) { #>
								...
								<# } else if ( field.type === 'upload' ) { #>
									...
									<figure class="d-kaigi-file-attachment" data-placeholder="<?php echo esc_attr__( 'No File Selected', 'theme-slug' ); ?>" >
										<# if ( field.default ) { #>
										...
										<# } else { #>
											<?php echo esc_attr__( 'No File Selected', 'theme-slug' ); ?>
										<# } #>
									</figure>

									<div class="actions">
										<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
										<button type="button" class="button upload-button" data-label="<?php echo esc_attr__( 'Add File', 'theme-slug' ); ?>" data-alt-label="<?php echo esc_attr__( 'Change File', 'theme-slug' ); ?>" >
											<# if ( field.default ) { #>
												<?php echo esc_attr__( 'Change File', 'theme-slug' ); ?>
											<# } else { #>
												<?php echo esc_attr__( 'Add File', 'theme-slug' ); ?>
											<# } #>
										</button>
										<# if ( field.default.id ) { #>
											<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
						...
						<# }); #>
						<button type="button" class="button-link repeater-row-remove"><?php echo esc_attr__( 'Remove', 'theme-slug' ); ?></button>
					</div>
				</li>
			</script>
			<?php
